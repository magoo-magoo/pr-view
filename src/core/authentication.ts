...
export const authenticate = async (ctx: GetServerSidePropsContext) => {
    ...
    if (ctx.query.code) {
        const url = `${githubAuthorizeBaseUrl}/access_token?client_id=${client_id}&client_secret=${clientSecret}&code=${ctx.query.code}`
        // authenticated
        ...
        // unauthenticated
        const autorizeUrl = `${githubAuthorizeBaseUrl}/authorize?client_id=${client_id}&scope=${scope}&redirect_uri=${`${redirectUrl}`}`
        if (!ctx.req) {
